...
{
    auto inds = TRTCommon::argsort(output.cbegin(), output.cend(), true);
...
inline vector<size_t> topK(const vector<T> inp, const size_t k)
{
    ...
    std::vector<size_t> inds = TRTCommon::argsort(inp.cbegin(), inp.cend(), true);
...
template <int C, int H, int W>
inline void readPPMFile(const std::string& filename, TRTCommon::PPM<C, H, W>& ppm)
{
