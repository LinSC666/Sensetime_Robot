...
public:
    ...
    void Publish(const MessageType& message) {
        ...
        memcpy(message_reference.message_sequence, message_sequence_str.c_str(),
               message_sequence_str.length());//cpy　data to the memory
        ...
        for (size_t i = 0; i < pools_.size(); ++i) {
            ...
            if (pool.get_free_memory() < serialized_message.length() + 256) {//如果小于消息的长度（放不下一帧消息）
                if (i == pools_.size() - 1) {
                    AppendNewPool();
                }
                continue;
            ...
            message_storage = pool.construct<MessageStorage>(message_sequence_str.c_str())(serialized_message,char_allocator);
            message_storage->reference_counter = subscriber_names_.size();

            memcpy(message_reference.pool_name, pool_name.c_str(),pool_name.length());
            break;
        }
        ...
        for (auto& subscriber_name : subscriber_names_) {
            ...
            while (!subscriber_queue->push(message_reference)) {
                ...
                CharAllocator char_allocator(pool.get_segment_manager());
                auto last_message_storage_ret = pool.find<MessageStorage>(last_message_reference_message_sequence.c_str());
                MessageStorage* last_message_storage =last_message_storage_ret.first;
...
private:
  ...
  //  std::atomic_uint64_t message_sequence_;
    std::atomic<std::uint64_t>  message_sequence_;
};
