...
AutomowerSafe::AutomowerSafe(const ros::NodeHandle& nodeh, decision_making::RosEventQueue* eq)
{
    ...
    cmd_sub = nh.subscribe("cmd_mode", 1, &AutomowerSafe::modeCallback, this);

    odom_pub = nh.advertise<nav_msgs::Odometry>("mower_odom", 5);
    pose_pub = nh.advertise<geometry_msgs::PoseStamped>("pose", 5);

    encoder_pub = nh.advertise<am_msgs::WheelEncoder>("wheel_encoder", 5);
    current_pub = nh.advertise<am_msgs::WheelCurrent>("wheel_current", 5);
    wheelPower_pub = nh.advertise<am_msgs::WheelPower>("wheel_power", 5);

    motorFeedbackDiffDrive_pub = nh.advertise<am_msgs::MotorFeedbackDiffDrive>("motor_feedback_diff_drive", 5);
    ...
    loop_pub = nh.advertise<am_msgs::Loop>("loop", 5);
    sensorStatus_pub = nh.advertise<am_msgs::SensorStatus>("sensor_status", 5);
     
    batStatus_pub  = nh.advertise<am_msgs::BatteryStatus>("battery_status", 5);
    
    navSatFix_pub = nh.advertise<sensor_msgs::NavSatFix>("GPSfix", 5);

    currentStatus_pub = nh.advertise<am_msgs::CurrentStatus>("current_status", 5);
...
void AutomowerSafe::simLoopCallback(const am_msgs::Loop& msg)
{
...
void AutomowerSafe::powerCallback(const am_msgs::WheelPower::ConstPtr& power)
{
