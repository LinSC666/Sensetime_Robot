...
const size_t kSubdivisionSize = 15;
...
void SimpleCoverageGlobalPlanner::initialize(std::string name,
    ...
    CalculateConnectedDomain(
            ...
            "connected_domain");//计算链接区域

    auto coverage_paths =
            CalculateCoveragePaths(gridmap_, "connected_domain", kRobotWidh);//kRobotWidh = 0.2,计算可覆盖的的路径
        ...
        while (node.ok() && !goal_checker_thread_stop_flag_) {
            {
                ...
                if (GetDistance(current_pose, current_goal_) > kGoalTolerance) {//kGoalTolerance = 0.8
                    continue;
                }
...
SimpleCoverageGlobalPlanner::CalculateCoveragePaths(grid_map::GridMap& gridmap,
                                                    ...
                                                    double kRobotWidh) {//kRobotWidh=0.2
    double resolution = gridmap.getResolution();
    double cell_step = kRobotWidh / resolution;
    grid_map::Matrix& layer_data = gridmap[layer];//layer just a string id="connected_domain"
    ...
    for (double row_index = 0; row_index < gridmap.getSize()(0);
        ...
        for (col_index = 0; col_index < gridmap.getSize()(1); ++col_index) {
            ...
            bool is_free = IsFreeInCircle(gridmap, layer, index, kMarginSize);//kMarginSize = 0.4//layer just a string id="connected_domain"
...
std::vector<grid_map::Index> SimpleCoverageGlobalPlanner::ConnectPaths(//***************************************//
    ...
    while (!paths.empty()) {
        ...
        double min_distance = GetDistance(last_index, paths.front().first);//获取直线距离
...
void SimpleCoverageGlobalPlanner::SubdivisonPath() {//***************************************//
    ...
    for (size_t i = 0; i < waypoints_.size(); ++i) {
        ...
        for (size_t j = 0; j < paths_[i].size(); j += kSubdivisionSize) {
            ...
            size_t end_index = j + kSubdivisionSize;//kSubdivisionSize = 20
...
bool SimpleCoverageGlobalPlanner::WorldToMap(double wx, double wy, double& mx,
    ...
    mx = (wx - origin_x) / resolution - kConvertOffset;//kConvertOffset = 0.5f
    my = (wy - origin_y) / resolution - kConvertOffset;//kConvertOffset = 0.5f
